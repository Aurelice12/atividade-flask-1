...
def nova_arma(nome, propriedade, tipo):
    ...
    cursor.execute(f"""
      INSERT INTO arma(nome, propriedade, tipo)
...
def listar_arma():
    conn = sqlite3.connect('bd')
    cursor = conn.cursor()
    values = cursor.execute("SELECT * FROM arma")
    resultado = []
    for item in values.fetchall():
        resultado.append({
            'id': item[0],
...
def nova_magia(nome, nivel, alcance):
    ...
    cursor.execute(f"""
      INSERT INTO magia(nome, nivel, alcance)
...
def listar_magia():
    conn = sqlite3.connect('bd')
    cursor = conn.cursor()
    values = cursor.execute("SELECT * FROM magia")
    resultado = []
    for item in values.fetchall():
        resultado.append({
            'id': item[0],
